Ext.define('FMS.app.applyAudit.module.ApplyAuditContractReceive', {
	// extend : 'Plugin.grid.Grid',
	extend : 'Plugin.grid.Show',
	alias : 'widget.FMSappapplyAuditmoduleApplyAuditContractReceive',
	...
	initComponent : function() {
		...
		_store.on('beforeload', function(_store, options) {
					var new_params = Ext.Object.fromQueryString("status=3&&riskCode=1");
					...
					if (selection.length <= 0) {
						...
						Ext.MessageBox.show({
							...
							fn: function(buttonId) {
								if (buttonId === "ok") {
									_this.openContractReceive(selection); 
								}
							}
						});
					}
				...
				handler : function(){
					...
					if (selection.length > 1 || selection.length <= 0) {
						...
						Ext.MessageBox.show({
							...
							fn: function(buttonId) {
								if (buttonId === "ok") {
									_this.contractRefuse(selection); 
								}
							}
						});
					}
...
//							            	allowBlank : false,
	...
	//提交接收数据
	openContractReceive : function(selection){
		var me = this;
		var url2 = StaticSetting.absUrl + "/mis/applyAudit/contractAuditList";
		me.visitApplyAuditByAjax(selection,url2,1,null);
		var url = StaticSetting.absUrl + "/mis/contract/contractAuditReceive";
		me.auditContractByAjax(selection,url,3);
	},
	
	auditContractByAjax : function(selection,url,status){
		var me = this;
		var param = new Array();
		for(var i=0; i<selection.length; i++) {
			param.push({"contractId":selection[i].data.contractId,"status":status});
		}
		Ext.Ajax.request({
            url: url,
            jsonData : param,
	...
	visitApplyAuditByAjax : function(selection,url,auditStatus,auditRemark){
		var me = this;
		var param = new Array();
		for(var i=0; i<selection.length; i++) {
			param.push({"contractId":selection[i].data.contractId,"auditStatus":auditStatus,
				"auditProgress":3,"auditRemark":auditRemark});
		}
		Ext.Ajax.request({
            url: url,
            jsonData : param,
	...
	//提交退回
	contractRefuse : function(selection){
		var me = this;
		var url2 = StaticSetting.absUrl + "/mis/applyAudit/contractAuditList";
		me.refuseReason(selection,url2,0);
//		var url = StaticSetting.absUrl + "/mis/contract/contractAuditReceive";
//		me.auditContractByAjax(selection,url,7);
	},
	
	//退回原因
	refuseReason : function(selection,url,status){
		var me = this;
		...
		var reasonWindow = Ext.create("Ext.window.Window",{
			...
			buttons : [{
				text : '确定',
				handler : function(){
					var auditRemark = reasonForm.getForm().findField("auditRemark").getValue();
					me.visitApplyAuditByAjax(selection,url,status,auditRemark);
					
					me.auditContractByAjax(selection,StaticSetting.absUrl + "/mis/contract/contractAuditReceive",7);

Ext.define('FMS.app.mis.module.window.CommonWindowClaim', {
	extend : 'Ext.window.Window',
	alias : 'widget.FMSAppMisModulewindowCommonWindowClaim',
	...
	getButtons : function(){
		...
		return [{
			...
			iconCls : 'icon-accept',
			handler : function() {
				var claimId = me.idParams.claimId;
				var contractId = me.idParams.contractId;
				var msg = null;
				msg = Ext.MessageBox.prompt("提示", "请输入审批人电话及意见", function(button, text) {
					if (button == 'ok') {
						me.submitStatus(claimId, text, contractId, 10, 1);
						msg.close();
						me.close();
			...
			handler : function() {
				...
				msg = Ext.MessageBox.prompt("提示", "请输入审批人电话及意见", function(button, text) {
					if (button == 'ok') {
						me.submitStatus(claimId, text, contractId, 10, 0);
						msg.close();
						Ext.Ajax.request({
				            url: StaticSetting.absUrl + "/mis/contractClaim/save",
				            params : {
				            	claimId : claimId,
			...
			handler : function() {
				...
				msg = Ext.MessageBox.prompt("提示", "请输入审批人电话及意见", function(button, text) {
					if (button == 'ok') {
						me.submitStatus(claimId, text, contractId, 10, 3);
						msg.close();
						Ext.Ajax.request({
				            url: StaticSetting.absUrl + "/mis/contractClaim/save",
				            params : {
	...
	submitStatus : function(claimId, text, contractId, auditProgress, auditStatus) {
		var me = this;
		Ext.Ajax.request({
            url: StaticSetting.absUrl + "/mis/applyAudit/contractClaimAudit",
            params : {
            	claimId : claimId,
            	contractId : contractId,
            	auditProgress : 10,

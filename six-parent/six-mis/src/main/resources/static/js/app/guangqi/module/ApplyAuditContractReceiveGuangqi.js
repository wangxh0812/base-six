Ext.define('FMS.app.guangqi.module.ApplyAuditContractReceiveGuangqi', {
	// extend : 'Plugin.grid.Grid',
	extend : 'Plugin.grid.Show',
	alias : 'widget.FMSappguangqimoduleApplyAuditContractReceiveGuangqi',
	...
	initComponent : function() {
		...
		_store.on('beforeload', function(_store, options) {
					var new_params = Ext.Object.fromQueryString("status=3&&riskCode=3");
...
//							            	allowBlank : false,
	...
	auditContractByAjax : function(selection,url){
		...
		var param = new Array();
		for(var i=0; i<selection.length; i++) {
			param.push({"contractId":selection[i].data.contractId,"status":3});
		}
		Ext.Ajax.request({
            url: url,
            jsonData : param,
	...
	visitApplyAuditByAjax : function(selection,url){
		...
		var param = new Array();
		for(var i=0; i<selection.length; i++) {
			param.push({"contractId":selection[i].data.contractId,"auditStatus":1,"auditProgress":3});
		}
		Ext.Ajax.request({
            url: url,
            jsonData : param,
